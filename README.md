...
**Ranked risk list for Phase 3** (highest judgement-call risk first): (1) `.empty-state` 3-way drift, (2) `.radar` max-width 340 vs 380, (3) dashboard `.task-card` vs list-page `.task-card`/`.survey-card` — structurally more complex, needs design judgement not just a value merge, (4) exit survey question blocks vs survey_detail.html's — see below.
